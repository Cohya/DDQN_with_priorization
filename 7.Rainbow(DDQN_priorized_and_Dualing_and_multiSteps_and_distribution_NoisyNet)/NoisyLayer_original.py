...
class NoisyLayer():
    ...
    def __init__(self, input_dims, output_dims, activation = tf.identity,
        ...
        self.mu_w = tf.Variable(initial_value = mu_w_0, trainable=True,  name = 'mu_w')
        
        self.sigma_w = tf.Variable(initial_value = sigma_w_0, trainable=True,  name = 'sigma_w')
        
        self.mu_b = tf.Variable(initial_value=mu_b_0, trainable=True,  name = 'mu_b')
        
        self.sigma_b = tf.Variable(initial_value=sigma_b_0, trainable=True, name = 'sigma_b')
        
        ## create tranable variable 
        self.eps_kernel = tf.Variable(initial_value= tf.zeros(
            shape = (self.p,self.q)),trainable=False, name = 'eps_w' )
        self.eps_bias = tf.Variable(initial_value= tf.zeros(
            shape =  (self.q)),trainable=False, name = 'eps_b' )
        
        self.trainabel_variables = [self.mu_w, self.sigma_w ,self.mu_b  , self.sigma_b]
    ...
    def __call__(self, inputs, training):
        ...
        w = self.mu_w + tf.math.multiply(self.sigma_w ,self.eps_kernel) # in pxq
        b = self.mu_b + tf.math.multiply(self.sigma_b , self.eps_bias) # in q
