...
class AnnDualing_Opt22(tf.keras.Model):
    def __init__(self, input_shape, K, num_atoms):
        """
        dictionary_details = {'LASTM':[[nuerons, activation],...],
                              'Dense': [[neurons, activation],....],
    ...
    def call(self, x):
        ...
        val = tf.tile(val, [1,K,1]) # (N, K, num_atoms)
...
ann2 = AnnDualing_Opt22(input_shape = 4, K= K, num_atoms = num_atoms)
q_s_a, cliped_dist  = ann.forward(x)
